...
def extract_audio_with_ffmpeg(input_path: str, output_path: str) -> str:
    ...
    # Check for bundled ffmpeg in .exe distribution
    if getattr(sys, 'frozen', False):
        # PyInstaller extracts to _MEIPASS temp folder
        meipass = getattr(sys, '_MEIPASS', os.path.dirname(sys.executable))
        bundled_ffmpeg = os.path.join(meipass, 'ffmpeg_bin', 'ffmpeg.exe')
        if os.path.exists(bundled_ffmpeg):
            ffmpeg_cmd = bundled_ffmpeg
        else:
            # Also check next to executable (alternative bundling)
            alt_ffmpeg = os.path.join(os.path.dirname(sys.executable), 'ffmpeg_bin', 'ffmpeg.exe')
            if os.path.exists(alt_ffmpeg):
                ffmpeg_cmd = alt_ffmpeg
